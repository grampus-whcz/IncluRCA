...
from IncluRCA.base.base_localizer_normal import BaseLocalizer
import argparse
from shared_util.file_handler import FileHandler
from shared_util.seed import *


class ALocalizer(BaseLocalizer):
...
if __name__ == '__main__':
    ...
    #                     default=f'{data_base_path}/temp_data/2022_CCF_AIOps_challenge/dataset/merge_multimodal/rca_multimodal_window_size_{window_size}.pkl',
    #                     type=str)
    # model_base_path = FileHandler.set_folder(f'{data_base_path}/model/a')
    # parser.add_argument("--model_path", default=f'{FileHandler.set_folder(model_base_path + "/checkpoint")}/main.pt',
    #                     type=str)
    ...
    parser.add_argument("--tidb_accuracy_th", default=0.5, type=float)
    ...
    # for normal data
    parser.add_argument("--use_pretraining", default=True, type=lambda x: x.lower() == "true", help="Enable or disable pre-training")
    parser.add_argument("--pretrain_epochs", default=10, type=int, help="Number of pre-training epochs")
    parser.add_argument("--pretrain_lr", default=0.001, type=float, help="Learning rate for pre-training")
    parser.add_argument("--na_pos_weight", default=1.0, type=float, help="Positive weight for normal/abnormal BCE loss (for handling imbalance)")
    parser.add_argument("--fault_loss_weight", default=1.0, type=float, help="Weight for the fault classification loss in the combined loss")
    parser.add_argument("--na_loss_weight", default=0.5, type=float, help="Weight for the normal/abnormal classification loss in the combined loss")

    parser.add_argument("--explainer_gpu", default=True, type=lambda x: x.lower() == "true")
    parser.add_argument("--explainer_epochs", default=100, type=int)
    parser.add_argument("--explainer_lr", default=0.1, type=float)
    parser.add_argument("--explainer_weight_decay", default=0.001, type=float)
