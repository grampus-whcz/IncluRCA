        ...
        for dataset_type, dataset_detail_dict in file_dict.items():
            ...
            for date in dataset_detail_dict['date']:
                ...
                for cloud_bed in dataset_detail_dict['cloud_bed']:
                    ...
                    self.process_traces(timestamp_list, data_base_path_dir, result_cloud_bed_path)                    
                                

    def process_traces(self, timestamp_list: list, data_base_path_dir: str, result_base_path: str):
        trace_pattern_list = []
        ...
        for root, dirs, files in os.walk(data_base_path_dir):
            ...
            for file in sorted_files:
                ...
                if file.endswith(".parquet"):
                    ...
                    self.logger.info(f'path: {data_base_path}.')                    
                    ...
                    for batch in batches:
                        for record in batch.to_pylist():
                            api_pattern = record.get("operationName")
                            ...
                            if serviceName not in self.config.data_dict['setting']['metric']['service_order']:
                                continue
                            ...
                            id_to_api_pattern[index][f'{trace_id}/{span_id}'] = f'{record.get("operationName")}'
                            temp_bucket_dict[api_pattern][index]['span_index_dict'][f'{trace_id}/{span_id}'] = len(temp_bucket_dict[api_pattern][index]['duration'])
                            temp_bucket_dict[api_pattern][index]['parent_span'].append(f'{trace_id}/{parent_span}')
                            temp_bucket_dict[api_pattern][index]['duration'].append(duration)
...
#     date = "2025-06-06"
#     timestamp_list = [TimeHandler.datetime_to_timestamp(f'{date} 00:00:00') + i * 60 for i in range(0, 24 * 60)]
#     raw_api_dao.process_traces(timestamp_list, "/root/shared-nvme/data_set/2025_CCF_aiops-live-benchmark/phaseone/2025-06-06/cloudbed/trace-parquet/trace_jaeger-span_2025-06-06_02-00-00.parquet", "/root/shared-nvme/work/code/RCA/IncluRCA/code/data_filter/CCF_AIOps_challenge_2025_api/dao/test/")
