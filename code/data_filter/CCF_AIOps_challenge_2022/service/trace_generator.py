            ...
            for date in dataset_detail_dict['date']:
                for cloud_bed in dataset_detail_dict['cloud_bed']:
                    ...
                    for feature_name in feature_df.keys():
                        ...
                        # print(raw_data[dataset_type][date][cloud_bed]['span_features'][feature_name].dtype)
                        # bug fix: convert to float if not already
                        if raw_data[dataset_type][date][cloud_bed]['span_features'][feature_name].dtype != float:
                            raw_data[dataset_type][date][cloud_bed]['span_features'][feature_name] = raw_data[dataset_type][date][cloud_bed]['span_features'][feature_name].astype(float)
                            ...
                            for position_type in ['upstream', 'current', 'downstream']:
                                for feature_type in ['<intensity>', '<duration>']:
                                    ...
                                    if feature_name not in temp_name_list:
                                        continue
                                    # if random.random() < prob and counter <= 200:
                                    counter += 1
